...
export const leadsRelations = relations(leads, ({ one, many }) => ({
  customerUser: one(users, { fields: [leads.customerUserId], references: [users.id] }),
  assignedAssistant: one(users, {
    fields: [leads.assignedAssistantId],
    references: [users.id],
    relationName: 'partnerAssistantLeads'
  }),
  recommendedPackage: one(packages, { fields: [leads.recommendedPackageId], references: [packages.id] }),
...
export const casesRelations = relations(cases, ({ one, many }) => ({
  customerUser: one(users, { fields: [cases.customerUserId], references: [users.id] }),
  assignedExpert: one(users, {
    fields: [cases.assignedExpertId],
    references: [users.id],
    relationName: 'expertCases'
  }),
  package: one(packages, { fields: [cases.packageId], references: [packages.id] }),
  payment: one(payments, { fields: [cases.paymentId], references: [payments.id] }),
  issues: many(issues),
